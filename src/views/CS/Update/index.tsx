...
export default function CSUpdate() {
 ...
 // state: 검색어 상태 //
 const [searchWord, setSerchWord] = useState<string>('');
...
const getCustomerResponse = (responseBody: GetCustomerResponseDto | ResponseDto | null ) => {
    const message = 
        ...
        const {profileImage , name, birth, charger, chargerName, address, location } = responseBody as GetCustomerResponseDto;
 ...
 const patchCustomerResponse = (responseBody: ResponseDto | null) => {
     ...
     responseBody.code === 'AF' ? '잘못된 입력입니다.' :
 ...
 const onUpdateClickHandler = async () => {
    if (!customerNumber) return;
     ...
     const requestBody: PatchCustomerRequestDto = {
         profileImage: url, name, birth, charger, address, location
     };
