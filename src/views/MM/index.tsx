  ...
  return (
    ...
    <div className='post-patch-input-container'>
      <div className='input-box'>
        ...
        <input className='input' type="text" value={name}  placeholder='용품명을 입력해주세요' onChange={onNameChangeHandler} />
      </div>
      {/* flex:1 =영역을 100% 차지 한다는 뜻 */}
      <div className='input-box' style={{flex:1}}>
        <div className='input-label'>용도</div>
        <input className='input' type="text" value={purpose} placeholder='용도를 입력해주세요' onChange={onPurposeChangeHandler} />
      </div>
      <div className='input-box'>
        <div className='input-label'>개수</div>
        <input className='input' type="text" value={count} placeholder='개수를 입력해주세요' onChange={onCountChangeHandler} />
      </div>
    </div>
    {/* button에 속한 자식요소 primary = e.g button 띄우고 primary */}
...
function PatchBox() {
  ...
  return (
    <div className='post-patch-box'>
      ...
      <div className='input-box'>
        <div className='input-label'>용품명</div>
        <input className='input' type="text" placeholder='용품명을 입력해주세요' />
      </div>
      {/* flex:1 =영역을 100% 차지 한다는 뜻 */}
      <div className='input-box' style={{flex:1}}>
        <div className='input-label'>용도</div>
        <input className='input' type="text" placeholder='용도를 입력해주세요' />
      </div>
      <div className='input-box'>
        <div className='input-label'>개수</div>
        <input className='input' type="text" placeholder='개수를 입력해주세요' />
      </div>
    </div>
    {/* button에 속한 자식요소 primary = e.g button 띄우고 primary */}
...
export default function MM() {
  ...
  // state: cookie 상태 //
  // 상태변경 아니고 쿠키 정보만 가져오는거니까 cookies만
  const [cookies] = useCookies(); 
  // state: 등록 및 수정 박스 뷰 상태 //

  const [showPostBox, setShowPostBox] = useState<boolean>(false);
  const [showPathBox, setShowPatchBox] = useState<Boolean>(false);
