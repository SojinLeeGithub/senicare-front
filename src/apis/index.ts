...
import { IdCheckRequestDto, SignInRequestDto, SignUpRequestDto, TelAuthRequestDto } from "./dto/request/auth";
...
export const telAuthRequest = async (requestBody: TelAuthRequestDto) => {
    const responseBody = await axios.post(TEL_AUTH_URL, requestBody)
        .then(responseDataHandler<ResponseDto>)
        .catch(responseErrorHandler);
    return responseBody;
};
...
export const getSignInRequest = async (accessToken: string) => {
    ...
    // Get 요청 방식이라 responsdBody 대신 header에 포함
...
export const getToolListRequest = async (accessToken: string) => {
    const responsdBody = await axios.get(GET_TOOL_LIST_API_URL, bearerAuthorization(accessToken))
