...
const TEL_AUTH = `${AUTH_MODULE_URL}/tel-auth`;
const TEL_AUTH_CHECK_URL = `${AUTH_MODULE_URL}/tel-auth-check`;
...
export const telAuthRequest = async (requestBody: TelAuthRequestDto) => {
    const responseBody = await axios.post(TEL_AUTH_CHECK_URL, requestBody)
        .then(responseDataHandler<ResponseDto>)
        .catch(responseErrorHandler);
    return responseBody;
};