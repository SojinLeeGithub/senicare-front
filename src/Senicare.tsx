...
// component: Sns Succss 컴포넌트 //
function SnsSuccess() {

    // state: Query paramter 상태 //
    const [queryParam] = useSearchParams();
    const accessToken = queryParam.get('accessToken');
    const expiration = queryParam.get('expiration')

    // state: cookie 상태 //
    // 배열이라서 setCookie 쓰려면 앞에 cookies 무조건 받아와야 한다.
    const [cookies, setCookie] = useCookies();

    // function: 네비게이터 함수 //
    const navigator = useNavigate();

    // effect: Sns Success 컴포넌트 로드시 accssToken expiration을 확인하여 로그인 처리 함수//
    useEffect(() => {
        if (accessToken && expiration) {
            // expiration = 초 단위라서 * 1000 해서 미리초 단위로 바꾼 것
            // Nuber = 숫자 타입으로 변경
...
const getSignInResponse = (responseBody: GetSignInResponseDto | ResponseDto | null) => {
    ...
    // effect: cookie의 accessTokn 값이 변경될 때마다 로그인 유저 정보 요청하는 함수 //
    useEffect(() => {
